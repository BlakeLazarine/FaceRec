...
public class WebcamPanel extends JPanel implements WebcamListener, PropertyChangeListener {
	...
	/*
	* This one was made by Blake Lazarine
	* It draws the allignment shapes and does real-time recognition
	*/
	public class FancyPainter extends DefaultPainter {
		@Override
		public void paintImage(WebcamPanel owner, BufferedImage image, Graphics2D g2) {
			super.paintImage(owner, image, g2);
			if(MyStuff.FaceRecRun.F != null && FaceRecRun.iterations > 10)
			{
				FaceRecRun.iterations = 0;
				FaceRecRun.project();
			}
			FaceRecRun.iterations++;
			g2.setColor(Color.cyan);
			int small = Math.min(image.getHeight(), image.getWidth());
			double scalingFactor = ((double) Math.min(owner.getHeight(),owner.getWidth()))/small;

			g2.fillOval((int) (owner.getWidth()/2 + scalingFactor * small / 9),owner.getHeight()/2, (int) (scalingFactor * 50 / 4.44),(int) (scalingFactor * 30 / 4.44));

			g2.fillOval((int) (owner.getWidth()/2 - scalingFactor * small / 9)-(int) (scalingFactor * 50 / 4.44),owner.getHeight()/2,(int) (scalingFactor * 50 / 4.44),(int) (scalingFactor * 30 / 4.44));

			g2.fillRect((int) (owner.getWidth()/2 - scalingFactor * small / 9),owner.getHeight()*3/4, (int) (scalingFactor * small *2/ 9),(int) (scalingFactor * 10 / 4.44));
		}
	}
	 ...
	 * Default painter.
	 */
	private final Painter defaultPainter = new FancyPainter();
